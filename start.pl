...
#
# start.pl 
# by Stella  Hartono (srhartono@ucdavis.edu)
# This program automatically run all scripts 
# to process all image files in PUT_SLIDES_HERE folder
#
#############################################################
...
if ($OS =~ m/MSWin/i) {
	...
	print "start.pl: your OS might not run this properly\n";
	$dir_user   = $ENV{HOME};
}
...
# Set config file containing slide folder path to be used by imageJ
open (my $path, ">$dir_user/path.cfg") or die "start.pl: Error: can't create $dir_user/path.cfg (probably permission issue): $!\n";
...
if ((not -e $file_macro and not -s $file_macro) or (not -e $file_jar)) {
	...
	copy("$file_jar_orig", "$dir_imagej") or die "start.pl: Error: can't copy $file_jar_orig (permission issue?): $!\n";
	my @macros = <$dir_bin*.ijm>;
	for (my $i = 0; $i< @macros; $i++) {
		print "start.pl: copying $macros[$i]...";
		copy("$macros[$i]", "$dir_imagej_macros") or die "start.pl: Error: can't copy $macros[$i] (permission issue?): $!\n";
		print "start.pl: successfully copied macros\n";
	}
}

# Run imageJ
# ImageJ will produce text files in result folder (see README.txt for description what each text files are)
# Get ImageJ result files and pipeline it to other scripts
...
die "start.pl: Error: imageJ result files not found in $dir_res_current\n" if (not defined($filenames_glob[0]) and not defined($filenames_cres[0]));

# Process result files using globulator.pl
...
# Make a summary using summarizer.pl (only relevant for human and monkey milk imagej, otherwise will produce blank text)
...
# Create graph from imageJ result coordinates
...
sub checkfile {
	...
	if (not -e $filename and not -s $filename) {
		die "start.pl: Error: $filename does not exist at bin directory!\n";
	}
}
